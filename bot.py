...
def cb(c):
    ...
    # ================= CREATE CREDIT =================
        ...
        with lock:
            cursor.execute("SELECT username, chat_id FROM users JOIN requests USING(user_id) WHERE user_id=?", (uid,))
        ...
        if row:
            bot.send_message(row[0], "❌️ Кредит отклонён | Свяжитесь с банком @ArabovBa")
